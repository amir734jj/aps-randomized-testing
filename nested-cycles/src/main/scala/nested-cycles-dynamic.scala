...
object nested_cycles_dynamic_implicit {
  ...
  type T_NESTED_CYCLES_DYNAMIC[T_T] = T_T;
}
    ...
    with C_NESTED_CYCLES_DYNAMIC[T_T,T_T]
{
  ...
  private class E_decl_assigned_in(anchor : T_Decl) extends Evaluation[T_Decl,T_NamesLattice](anchor,anchor.toString()+"."+"decl_assigned_in") with CircularEvaluation[T_Decl,T_NamesLattice] {
    def lattice : C_LATTICE[T_NamesLattice] = t_NamesLattice;
  ...
  private class E_decls_assigned_in(anchor : T_Decls) extends Evaluation[T_Decls,T_NamesLattice](anchor,anchor.toString()+"."+"decls_assigned_in") with CircularEvaluation[T_Decls,T_NamesLattice] {
    def lattice : C_LATTICE[T_NamesLattice] = t_NamesLattice;
    ...
    for (anchor <- t_Result.t_Decl.nodes) anchor match {
      case p_decl(v_d,v_id,v_ty) => {
        { val cond = v_not(new M__basic_14[ T_String,T_NamesLattice](t_String,t_NamesLattice).v_in(v_id,v_decl_assigned_in(v_d)));
          if (cond) {
            collection = t_Messages.v_combine(collection,t_Messages.v_single(new M__basic_18[ T_String](t_String).v__op_ss(v_id," was not 'assigned.'")));
          }
        }
      }
      case _ => {}
